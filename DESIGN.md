...
## 4. 数据管道与测试脚本 (Data Pipeline & Automation)

- **解析脚本 (`src/data/parse_draft.py`)**：
  - 从 `draft.txt` 自动化提取 431 道 6选2 试题。
  - 结合 NLTK WordNet 与同义词算法自动提取答案，并同步补充 379 组等价词对至 `words.csv`。
- **测试脚本 (`src/data/verify_questions.py`)**：
  - 自动化验证 430 道题目的题干完整性、6 选项无空/无重复、答案必须存在于选项中。
  - 验证通过率：**100% PASS**。